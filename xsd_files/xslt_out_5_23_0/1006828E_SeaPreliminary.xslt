...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sp="urn:customs.ru:Information:CustomsDocuments:SeaPreliminary:5.23.0" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cat_ru="urn:customs.ru:CommonAggregateTypes:5.22.0" xmlns:clt_ru="urn:customs.ru:CommonLeafTypes:5.10.0" xmlns:RUScat_ru="urn:customs.ru:RUSCommonAggregateTypes:5.22.0" xmlns:RUSclt_ru="urn:customs.ru:RUSCommonLeafTypes:5.21.0" xmlns:RUDECLcat="urn:customs.ru:RUDeclCommonAggregateTypesCust:5.23.0" xmlns:cltESAD_cu="urn:customs.ru:CUESADCommonLeafTypes:5.17.0" xmlns:cltpi_ru="urn:customs.ru:Information:PriorInformation:PriorCommonLeafTypes:5.17.0" xmlns:catpi_ru="urn:customs.ru:Information:PriorInformation:PriorCommonAggregateTypes:5.23.0">
	...
	<xsl:template match="sp:ConsignmentDetails">
		<tr class="title" align="center">
			<td class="bordered" colspan="4" style="font-style: oblique; background-color: #999">Сведения о товарной партии № <xsl:value-of select="sp:ObjectOrdinal"/></td>
		</tr>
		<tr>
			<td class="bordered">
		...
		<tr>
			...
			<td class="bordered">
				...
				<xsl:value-of select="sp:DeclarationFeatureCode"/>
				<!--Кол-во отгрузочных спецификаций<br/>
				<xsl:value-of select="sp:SpecificationNumber"/ удален-->
			</td>
			<td class="bordered">
				Код исп. трансп. (перевоз.), коммерч. и (или) иных док-тов в качестве ТД<br/>
				<xsl:value-of select="sp:DocUsageCode"/>
				<!--Кол-во листов отгр.спецификаций<br/>
				<xsl:value-of select="sp:SpecificationListNumber"/ удален-->
			</td>
		</tr>
		<tr>
			<td class="bordered">
			...
			<td class="bordered">
				...
				<xsl:apply-templates select="sp:DispatchCountryCode"/>
			</td>
			<td class="bordered">
				Страна назначения<br/>
				<xsl:apply-templates select="sp:DestinationCountryCode"/>
			</td>
		</tr>
		<tr>
			<td class="bordered">
				Общая стоимость товаров<br/>
				<xsl:apply-templates select="sp:TotalAmount" mode="spaced"/>
			</td>
			<td class="bordered">
			...
			<td class="bordered">
				...
				<xsl:value-of select="sp:PageQuantity"/>
			</td>
			<td class="bordered">
				Таможенный орган назначения<br/>
				<xsl:value-of select="sp:DestinationCustomsOfficeCode"/>
			</td>
		</tr>
		...
		<tr>
			<td class="bordered">
				Место погрузки товаров на судно:<br/>
				<xsl:apply-templates select="sp:LoadingLocationDetails"/>
			</td>
			<td class="bordered">
				Признак выгрузки<br/>
			...
			<td class="bordered">
				Место выгрузки товаров с судна: 
				<xsl:apply-templates select="sp:UnLoadingLocationDetails"/>
			</td>
			<td class="bordered">
				Место первоначального отправления товаров:<br/>
				<xsl:apply-templates select="sp:FactLocationDepartureDetails"/>
			</td>
		</tr>
		<tr>
			<td class="bordered" colspan="4">
		...
		<xsl:if test="sp:UnionCarrierDetails">
			...
			<xsl:for-each select="sp:UnionCarrierDetails">
				<tr>
					<td class="bordered" colspan="2">
						Порядковый номер: 
		...
		<xsl:if test="sp:SignatoryRepresentativeDetails">
			<tr class="title">
			...
			<tr>
				...
				<td class="bordered" colspan="2">
					Код вида док-та: 
					<xsl:value-of select="sp:SignatoryRepresentativeDetails/catpi_ru:RegisterDocumentIdDetails/catpi_ru:DocKindCode"/>
				</td>
			</tr>
			<xsl:apply-templates select="sp:SignatoryRepresentativeDetails/catpi_ru:RegisterDocumentIdDetails" mode="certif"/>
			<tr class="title">
			...
			<tr>
				<td class="bordered">
					Реквизиты: 
					<xsl:apply-templates select="sp:SignatoryRepresentativeDetails/catpi_ru:RepresentativeContractDetails" mode="doc"/>
				</td>
				<td class="bordered">
					Дата начала срока, в течение которого док-т имеет силу: 
					<xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="sp:SignatoryRepresentativeDetails/catpi_ru:RepresentativeContractDetails/RUScat_ru:DocStartDate"/></xsl:call-template>
				</td>
				<td class="bordered">
					Дата окончания срока, в течение которого док-т имеет силу: 
					<xsl:call-template name="russian_date"><xsl:with-param name="dateIn" select="sp:SignatoryRepresentativeDetails/catpi_ru:RepresentativeContractDetails/RUScat_ru:DocValidityDate"/></xsl:call-template>
				</td>
				<td class="bordered">
					Код вида док-та: 
					<xsl:value-of select="sp:SignatoryRepresentativeDetails/catpi_ru:RepresentativeContractDetails/RUScat_ru:DocKindCode"/>
				</td>
			</tr>
		</xsl:if>
		<xsl:apply-templates select="sp:SignatoryPersonDetails"/>
	...
	<xsl:template match="sp:SignatoryPersonDetails">
		<tr class="title">
	...
	<xsl:template match="catpi_ru:GoodsDisinfectionDetails">
		<tr>
			<td class="bordered" colspan="4">
				Проведено:<br/>
				<xsl:apply-templates select="catpi_ru:DisinfectionIndicator" mode="indicator_yn"/>
			</td>
		</tr>
	...
	<xsl:template match="sp:CustomsIdentificationDetails">
		<tr>
			<td class="bordered">
				Количество:<br/>
	...
	<xsl:template match="sp:BorderTransportDetails">
		<tr>
			...
			<td class="bordered">
				<!--Кол-во<br/>
				<xsl:value-of select="sp:TransportMeansQuantity"/ удален-->
				№ п/п<br/>
				<xsl:value-of select="sp:ObjectOrdinal"/>
			</td>
			<td class="bordered">
	...
	<xsl:template match="sp:LoadingLocationDetails | sp:UnLoadingLocationDetails | sp:FactLocationDepartureDetails">
	...
	<xsl:template match="*" mode="comma">
		<xsl:for-each select="*">
			...
			<xsl:choose>
				...
				<xsl:when test="contains(local-name(.), 'RegistryKind')">
					<xsl:choose>
						<xsl:when test=".='1'">Государственный реестр (Государственный судовой реестр)</xsl:when>
						<xsl:when test=".='2'">Международный реестр судов</xsl:when>
						<xsl:when test=".='3'">Бербоут-чартерный реестр</xsl:when>
						<xsl:when test=".='4'">Реестр маломерных судов (судовой реестр)</xsl:when>
					</xsl:choose>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="."/>
